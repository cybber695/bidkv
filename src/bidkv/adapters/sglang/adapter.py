...
class SGLangAdapter(FrameworkAdapter):
    ...
    def _execute_baseline_actions(self, actions: list[CompressionAction]) -> int:
        ...
        execution path (execute_compression) to ensure a single-variable
        experiment — the only difference between strategies is *which*
        requests are selected as victims, not *how* the KV is freed.
        Routing evict vs compress to different mechanisms would introduce
        a confounding variable and compromise fairness.
        """
        total_freed = 0
