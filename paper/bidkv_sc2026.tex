...
per-request reclamation costs must be \emph{estimated} at runtime from
serving state, and the selection must execute within the engine's scheduling
loop under strict latency constraints---bridging the gap between
offline formulation and online serving-native execution.
...
coordinates selection across the batch (Section~\ref{sec:design}).
...
The completion term protects near-complete requests whose reclamation is
expensive; the prompt-length term penalizes large-prompt victims whose
reclamation cost is proportional to prompt size; and the preemption term
...
recovery path, the policy preserves output correctness; the solver's degree
of freedom is \emph{which} requests to reclaim and \emph{how often}.
This degree of freedom directly governs admission responsiveness: cost-aware
...
fairness-aware solver extensions is left to future work.
...
in \emph{which} requests are reclaimed and \emph{how often}---choices that
directly govern how quickly queued requests receive KV allocation and,
consequently, first-token latency and SLO attainment.
