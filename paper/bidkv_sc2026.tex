...
\vspace{2pt}
\begin{subfigure}[b]{\columnwidth}
  \includegraphics[width=\columnwidth]{figures/fig1_intro_evidence_panel_b.pdf}
  \caption{Snapshot of a single KV-pressure event under vLLM's native LIFO
    policy (32 concurrent requests).  Each dot is an active candidate;
    the red\,\texttimes{} marks the LIFO victim.  LIFO selects the request
    with the \emph{smallest} KV footprint (fewest tokens to free) while
    ignoring candidates in the ideal zone (upper-left: large footprint,
    low invested decode work), directly wasting the preemption event.}
...
\caption{\textbf{The victim-selection problem under KV pressure.}
  ...
  (b)~A snapshot from the PE (LIFO) baseline: LIFO frequently selects
  ``expensive'' victims (high invested decode work or small KV footprint)
  while ignoring optimal candidates, directly causing severe recomputation
  waste and tail-latency degradation.}
