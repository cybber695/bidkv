...
\begin{subfigure}[b]{0.50\textwidth}
  ...
  \caption{Victim choices by three strategies across 150 KV-pressure snapshots
    (mixed ShareGPT, rate\,=\,3.8\,req/s).
    Grey dots are all active candidates; coloured dots are each strategy's
    selected victim per snapshot.
    LIFO\,(red) always picks the newest, smallest-footprint request
    (bottom-left cluster).
    Largest-first\,(blue) always picks maximum KV footprint, but also the
    highest decode-work requests (top-right edge).
    BidKV\,(green) distributes across the high-KV, moderate-cost band,
    maximising utility $U\!=\!Y/(1{+}0.5c)$.}
  \label{fig:intro:lifo}
\end{subfigure}
\caption{\textbf{The victim-selection problem under KV pressure.}
  (a)~Reclamation opportunity spans orders of magnitude across concurrently
  active requests at each pressure event.
  (b)~Victim selections across 150 snapshots: LIFO consistently wastes
  preemption events (bottom-left); Largest-first ignores recompute cost
  (top-right edge); BidKV targets the high-KV, moderate-cost band.}
