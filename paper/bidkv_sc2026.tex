...
\begin{subfigure}[b]{0.50\textwidth}
  ...
  \caption{Victim-selection profiles for three strategies across 150\,KV-pressure
    snapshots (mixed ShareGPT, rate\,=\,3.8\,req/s).
    \textbf{Top}: CDF of each strategy's chosen victim's KV-footprint
    \emph{rank} within its snapshot (1\,=\,largest candidate).
    LIFO\,(red) is spread uniformly, recovering little KV ($\bar{r}\!=\!0.21$).
    Largest-first\,(blue) always picks rank\,1 (vertical step at\,1.0).
    BidKV\,(green) concentrates near rank\,1 ($\bar{r}\!=\!0.96$), matching
    Largest-first in KV recovery.
    \textbf{Bottom}: CDF of chosen victim's completion ratio\,$c$
    (0\,=\,newest, 1\,=\,almost done).
    BidKV's curve is \emph{left-shifted} vs.\ Largest-first
    ($\bar{c}\!=\!0.64$ vs.\ 0.74), confirming it prefers lower-cost victims
    that free nearly the same KV.}
...
\caption{\textbf{The victim-selection problem under KV pressure.}
  ...
  (b)~CDFs of KV rank and completion ratio $c$ of chosen victims:
  LIFO wastes preemption events (low KV, $\bar{r}\!=\!0.21$);
  Largest-first maximises KV but ignores cost ($\bar{c}\!=\!0.74$);
  BidKV recovers nearly as much KV ($\bar{r}\!=\!0.96$) at lower
  recompute cost ($\bar{c}\!=\!0.64$).}
