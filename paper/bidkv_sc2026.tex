...
(rate~=~3.8\,req/s), we examine the spread of an \emph{analysis-only
reclamation-opportunity score}
\begin{equation}
R_i = B_i \cdot (1 - p_i)
\label{eq:reclamation_opportunity}
\end{equation}
where $B_i$ is the number of KV tokens currently held by candidate~$i$
(and therefore freed in full upon preemption under recompute-fallback
semantics), and $p_i = g_i / G_i$ is a post-hoc completion fraction, with
$g_i$ denoting the output tokens generated so far and $G_i$ the realized final
output length from the same run. This score is \emph{analysis-only} and
\emph{method-agnostic}: it references no scheduling formula and simply captures
the intuition that a favorable eviction target should both free substantial KV
space and have invested relatively little decode work.

The empirical answer is unambiguously yes: reclamation opportunities are highly
heterogeneous. In 99.6\% of observed events, the highest- and
lowest-opportunity candidates differ by at least 10$\times$ in $R$
(i.e., $\max R / \min R \geq 10$); the median spread is 128$\times$, and
94.8\% of events exceed 20$\times$. This pervasive heterogeneity arises from
the interaction of two independent dimensions: KV footprint varies widely
across requests of different prompt lengths, and completion fraction is spread
across the $[0,1]$ interval as requests enter and exit at different lifecycle
stages. Order-only heuristics that ignore both dimensions---such as LIFO, which
selects the most recently admitted request---therefore routinely encounter
events where the chosen victim is far from the highest-opportunity candidate
under this proxy. The victim-selection problem is therefore not merely
theoretical; it appears repeatedly in realistic traces, and this heterogeneity
motivates the two capabilities identified above: explicit per-request
reclamation-cost signals and cross-request coordination.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/fig1_motivation_1panel.pdf}
...
$R_i = B_i \cdot (1 - p_i)$, where $B_i$ is the number of KV tokens currently
held and $p_i = g_i / G_i$ is a post-hoc completion fraction
($G_i$ is the realized final output length from the same run). $R$ is an
analysis-only, method-agnostic proxy that combines reclaimable capacity and
progress without referencing any scheduling formula. Spread is pervasive:
99.6\% of 250 events exceed 10$\times$, the median is 128$\times$, and 94.8\%
exceed 20$\times$. Real pressure events therefore expose sharply heterogeneous
reclamation opportunities, so victim selection cannot be safely reduced to
recency or queue order alone.}
\label{fig:motivation}
\end{figure}
