...
\begin{subfigure}[b]{0.50\textwidth}
  ...
  \caption{Victim selection by four strategies on the \emph{same} 32-request
    ...
    chosen victim.
    LIFO\,(\texttimes) picks the newest, smallest-footprint request
    (bottom-left), freeing minimal KV while incurring full prompt recompute.
    Largest-first\,(\(\blacklozenge\)) maximises KV freed but also
    maximises the invested decode work (195~tokens already generated).
    BidKV\,($\bigstar$) selects for $U\!=\!Y/(1{+}0.5c)$: nearly equal
    KV recovery to Largest-first, at roughly half the recompute cost
    ($c\approx0.24$ vs.\ $c\approx0.98$)---the utility-optimal choice.
    Random\,(\(\circ\)) is opaque and non-repeatable.}
