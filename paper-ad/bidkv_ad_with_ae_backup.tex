...
\begin{itemize}
  \item $T_1$ \textbf{Trace generation.}
    ...
    SHA-256-verified JSON trace files to \texttt{data/traces/}.
...
\textbf{Step 1 --- Create and activate a conda environment:}
\begingroup\footnotesize\begin{verbatim}
conda create -n bidkv python=3.10 -y
conda activate bidkv
\end{verbatim}
...
\textbf{Step 2 --- Install inference frameworks and BidKV:}
\begingroup\footnotesize\begin{verbatim}
pip install vllm==0.17.1
pip install -e "/path/to/bidkv[dev]"
\end{verbatim}
\endgroup

\textbf{Step 3 --- If using a local HuggingFace model cache, export:}
\begingroup\footnotesize\begin{verbatim}
...
\textbf{Step 4 --- Generate frozen request traces (seed~42):}
\begingroup\footnotesize\begin{verbatim}
python -m bidkv.experiments.vllm.freeze_traces \
    --mode formal --use-frozen-rates \
    --output-dir experiments/vllm/traces/
\end{verbatim}
\endgroup
Produces six SHA-256-verified trace files and
\texttt{manifest.json} in \texttt{experiments/vllm/traces/}.

\artcomp

The vLLM experiment matrix covers 5 strategies $\times$ 2 workloads
$\times$ 3 arrival rates $\times$ 3 runs = 90 runs.
Launch the full matrix with:

\begingroup\footnotesize\begin{verbatim}
cd /path/to/bidkv
python -m bidkv.experiments.vllm.runner \
    --strategies bidkv,largest-first,preempt-evict,\
preempt-evict-sjf,static-random \
...
The runner starts a fresh vLLM server per strategy (injecting
the BidKV adapter for non-PE strategies via
\texttt{BIDKV\_STRATEGY} env variable), replays the frozen trace
with a Poisson open-loop arrival model, and writes one JSON result
file per run.
\textbf{Do not change} \texttt{--num-gpu-blocks-override 600}:
this cap (9\,600 tokens KV capacity) is the critical parameter
that induces sustained KV pressure and enables strategy
differentiation.

\textbf{Task dependencies:} trace generation must complete before
any runner invocation.
Mixed and long-context jobs are independent and can run in
parallel on separate GPUs if available.

\textbf{SGLang portability slice ($T_3$):} pending; results for
Table~IV are not included in this artifact version.

\artout

After all runs complete, invoke the analysis script:

\begingroup\footnotesize\begin{verbatim}
python -m bidkv.experiments.vllm.analysis \
    --results-dir results/vllm_full/ \
    --output-dir results/vllm_analysis/
\end{verbatim}
\endgroup

The script reads every result file
(named \texttt{strategy\_\_{workload}\_\_rate\{r\}\_\_r\{i\}.json}),
computes P95/P50 TTFT and TPOT, throughput, and SLO
attainment from raw per-request data, and writes CSV tables and
PDF figures to \texttt{results/vllm\_analysis/}.

\textbf{How to verify the key claims against the paper:}

\begin{itemize}
  \item \textbf{$C_1$/$C_2$ (utility-ranked victim selection):}
    In the mixed-workload CSV at rate~3.8\,req/s (Table~II),
    BidKV should show P95~TTFT $\approx$631\,ms and
    SLO(300\,ms) $\approx$83.2\,\%, lowest P95~TTFT among all
    five strategies.

  \item \textbf{$C_4$ (rate sensitivity, Table~II, Figure~4):}
    Across rates $\{2.0, 3.8, 5.7\}$\,req/s, BidKV should rank
    first or second on P95~TTFT and SLO attainment.
    At rate~2.0 BidKV should lead all four metrics.

  \item \textbf{$C_4$ (long-context, Table~III):}
    At rate~0.70\,req/s, BidKV should show the lowest P95~TTFT
    ($\approx$30{,}143\,ms) among all five strategies.

  \item \textbf{$C_3$ (portability, Table~IV):}
    SGLang results are pending and will be added before
    camera-ready submission.
\end{itemize}

Minor run-to-run variance ($\pm$2--5\,\%) is expected due to
GPU scheduling non-determinism; relative rankings should be
